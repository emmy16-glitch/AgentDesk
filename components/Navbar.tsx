...
export default function Navbar() {
  ...
  return <nav className="top-nav"><div className="nav-inner"><a className="brand" href="/"><CubeMark /><span>Agent<span>Trust</span></span></a><div className="nav-links"><a className="current" href="/">Marketplace</a><a href="/#agents">Explore</a><a href="/#how-it-works">How it works</a><a href="/agents/healthguard-ai">Docs</a></div><div className="nav-controls"><button type="button" className="network"><b />BSC Testnet<ChevronDown size={15} /></button><button type="button" className="account" onClick={openConnect}><Wallet size={18} /><span>{shortAddress}</span>{connected && <ChevronDown size={15} />}</button></div></div></nav>;
}
