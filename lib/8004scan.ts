...
// These are audited discovery anchors, not a static marketplace catalogue. Every request
// resolves them again from the live BSC ERC-8004 registry and only keeps an anchor if its
// current metadata still contains evidence for the intended category.
...
export async function discoverAcrossRequiredCategories(limitPerCategory = 5): Promise<{
  ...
  const [anchorSettled, searchSettled] = await Promise.all([
    Promise.allSettled(
      categories.map(async (category) => ({
        category,
        agent: await resolveCoverageAnchor(category, COVERAGE_ANCHORS[category]),
      })),
    ),
    Promise.allSettled(
  ...
  // Audited anchors are inserted first so the task flow starts with a currently resolved,
  // category-specific identity when one is live. Semantic discovery still contributes
  // additional real candidates after the anchor.
